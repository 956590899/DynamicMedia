...
CUSTOM_SCAN_DIR           = ""     # 自定义扫描目录：留空使用默认目录，否则使用指定目录（示例：CUSTOM_SCAN_DIR = "D:\Music"）

# 本地歌词配置
LOCAL_LYRIC_ENABLED       = 0      # 本地歌词开关：0 = 关闭，1 = 开启（开启时优先获取本地歌词）
LOCAL_LYRIC_DIR           = ""     # 自定义本地歌词目录：留空使用根目录LRC文件夹，否则使用指定目录（示例：LOCAL_LYRIC_DIR = "D:\Lyrics"）

# 在线歌词配置
NETWORK_LYRIC_ENABLED     = 0      # 联网获取歌词开关：0 = 关闭，1 = 开启
...
try:
    ...
    print("mutagen库未安装，无法读取音频文件内嵌歌词")
...
def search_audio_file(artist_name, song_name):
    ...
    # 添加自定义扫描目录
    if CUSTOM_SCAN_DIR and os.path.exists(CUSTOM_SCAN_DIR) and CUSTOM_SCAN_DIR not in added_dirs:
        search_dirs.append(CUSTOM_SCAN_DIR)
        added_dirs.add(CUSTOM_SCAN_DIR)
...
def get_local_lyric(artist_name, track_name):
    ...
    try:
        ...
        # 优先使用根目录的LRC文件夹（保持权威性）
        lrc_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "LRC")
        lyric_dirs.append(lrc_dir)
        
        # 如果设置了自定义目录，也添加到搜索列表
        if LOCAL_LYRIC_DIR:
            lyric_dirs.append(LOCAL_LYRIC_DIR)
        
        # 确保所有歌词目录存在
        for lyric_dir in lyric_dirs:
        ...
        for lyric_dir in lyric_dirs:
            for filename in possible_filenames:
                # 尝试不同的编码读取文件
                lyric_path = os.path.join(lyric_dir, filename)
...
def get_lyric(artist_name, track_name, file_path=""):
    ...
    if NETWORK_LYRIC_ENABLED == 0:
        ...
        try:
            ...
            print(f"[歌词搜索过程] 搜索URL: {search_url}")
            
            # 发送搜索请求
            response = urllib.request.urlopen(search_url, timeout=5)
            content = response.read().decode('utf-8')
            data = json.loads(content)
            
            # 如果有结果，获取歌词
            if data:
                print(f"[歌词搜索过程] 找到 {len(data)} 个搜索结果")
                first = data[0]
                print(f"[歌词搜索过程] 选择第一个结果: {first.get('trackName', '未知')}")
                # 获取歌词
                lyric_url = f"https://lrclib.net/api/get/{first['id']}"
                print(f"[歌词搜索过程] 获取歌词URL: {lyric_url}")
                lyric_response = urllib.request.urlopen(lyric_url, timeout=5)
                lyric_content = lyric_response.read().decode('utf-8')
                lyric_data = json.loads(lyric_content)
                # 返回歌词，优先使用同步歌词，其次使用普通歌词
                lyric = lyric_data.get('syncedLyrics') or lyric_data.get('plainLyrics') or ""
                if lyric:
                    print("[歌词搜索过程] 成功获取联网歌词")
                else:
                    print("[歌词搜索过程] 联网未找到歌词")
                return lyric
            else:
...
class AudioCaptureThread(QThread):
    ...
    def __init__(self):
        ...
        self.chunk_size = 1024  # 每次捕获的音频数据大小
...
class DynamicIslandWidget(QWidget):
    ...
    def is_fullscreen_or_maximized(self):
        ...
        try:
            has_fullscreen_or_maximized = False
            fullscreen_window_info = ""
            
            # 回调函数，用于枚举所有窗口
            def enum_windows_proc(hwnd, lParam):
                nonlocal has_fullscreen_or_maximized, fullscreen_window_info
                ...
                # 先检查窗口标题是否为系统窗口
                if window_title in ['Program Manager', '', 'File Explorer', 'Task Switching', 'Windows 输入体验'] or 'NVIDIA GeForce Overlay' in window_title:
                    return True  # 继续枚举
                ...
                if is_maximized:
                    has_fullscreen_or_maximized = True
                    fullscreen_window_info = f"{window_title} ({process_name}) [最大化]"
                    # 继续枚举，检查是否有全屏窗口
            ...
            return has_fullscreen_or_maximized, fullscreen_window_info
        except Exception as e:
            print(f"全屏检测错误: {e}")
        return False, ""
    ...
    def update_media_info(self, media_info):
        ...
        if is_new_song:
            try:
                ...
                print(f"解析到的媒体信息：歌曲名='{song_name}', 歌手名='{artist_name}', 当前歌曲名='{self.current_song}', 当前歌手名='{self.current_artist}'")
                ...
                print("解析到的媒体信息：歌曲名='{}', 歌手名='{}', 当前歌曲名='{}', 当前歌手名='{}".format(
                    song_name.encode('utf-8', 'replace').decode('utf-8'),
                    artist_name.encode('utf-8', 'replace').decode('utf-8'),
                    self.current_song.encode('utf-8', 'replace').decode('utf-8'),
                    self.current_artist.encode('utf-8', 'replace').decode('utf-8')
                ))
        ...
        if FLOAT_LYRIC_ENABLED == 1:
            ...
            if self.current_parsed_lyric and self.is_playing:
                ...
                if adjusted_seconds >= 0.5:
                    ...
                    if not current_lyric and next_lyric_time:
